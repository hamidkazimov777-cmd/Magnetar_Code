...
## Установка с нуля, по шагам

Если ты никогда не пользовался терминалом — это нормально, здесь всё по порядку.

**Шаг 1. Открой терминал.**
На Mac нажми `Cmd + Пробел`, напиши `Терминал` и нажми Enter. Откроется чёрное
окно — в него мы будем вводить команды. Каждую команду вводи отдельно и жми
Enter.

**Шаг 2. Проверь, что установлен Node.js.**

```bash
node -v
```

Если ответило что-то вроде `v22.1.0` — всё хорошо, число должно быть **20 или
больше**. Если написало `command not found` или число меньше 20 — зайди на
[nodejs.org](https://nodejs.org), скачай версию LTS, установи её как обычную
программу и открой терминал заново.

**Шаг 3. Установи Magnetar.**

```bash
npm i -g magnetar-code
```

Подожди полминуты. Если увидишь строки со словом `npm error` и `EACCES` —
у твоего пользователя нет прав на установку; тогда выполни ту же команду с
`sudo` в начале и введи пароль от компьютера.

**Шаг 4. Проверь, что установилось.**

```bash
magnetar --version
```

Должен появиться номер версии. Если написало `command not found` — закрой
терминал и открой заново.

**Шаг 5. Подключи свой ключ.**

```bash
magnetar provider
```

Стрелками ↑ ↓ выбери провайдера (например, OpenRouter), нажми Enter, вставь
свой API-ключ и снова Enter. Потом выбери модель. Дальше спросит модель для
анализа проекта — выбери ту же или посильнее.

Ключ нужен свой: в программе его нет, она подключается к тому сервису, у
которого ты купил доступ.

**Шаг 6. Работай.**

Зайди в папку со своим проектом и запусти:
...
Внутри нажми `/` — увидишь список команд. Чтобы выйти, нажми `Ctrl + C` два
раза.
...
Если раньше ты подключал версию из папки с исходниками (`npm link`), сначала
отвяжи её:

```bash
npm unlink -g magnetar-code
```
...
Эта команда проверит ключ, соединение с провайдером и скажет, что именно не
так.

**Первый раз?** [Полное руководство](docs/GUIDE.ru.md) — каждая команда с
примерами, от установки до выпуска приложения.
